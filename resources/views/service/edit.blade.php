        ...
        <div class="col-md-12">
			...
			<div class="row">
				<div class="col-sm-4">
					...
					<img id="image_blog" src="{{ url('/') }}/public/images/{{ $service->image }}" width="100%">
					@else
					...
					<div class="form-group row">
					    ...
					    <div class="col-md-12">
					    	@foreach($iconList as $icon)
					    		...
					    		<span class="service selected" data-icon="{{ $icon }}"><i class="fa fa-{{ $icon }} icon-size"></i></span>
					    		@else
				...
				<div class="col-sm-8">
					<form action="{{ url('services/'.$service->id) }}" method="POST" class="form-horizontal">
                		{{ method_field('PUT') }}
                		{{csrf_field()}}
						<input type="hidden" id="image_field" name="image" value="{{ $service->image }}">
						<input type="hidden" id="icon_field" name="icon" value="{{ $service->icon }}">
...
<script>
    ...
    var html_sub_content = '<?php echo preg_replace('/(\>)\s*(\<)/m', '$1$2', str_replace(PHP_EOL, '', $service->sub_content)); ?>';
    ...
    var html_content = '<?php echo preg_replace('/(\>)\s*(\<)/m', '$1$2', str_replace(PHP_EOL, '', $service->content)); ?>';
